...
void main() {
	...
	if (isEyeInWater == 0 && depth < 1.0) {
		float transmittance = fastExp(-fogDist * 2e-3);
		...
		sceneData += mix(vec3(0.99, 0.88, 0.27), vec3(0.04, 0.02, 0.05), darkEnd * 0.94) * oneMinus(transmittance);
	}
	...
	#ifdef BLOOMY_FOG
		fogTransmittance = saturate(VFData.a * 0.7 + 0.3);

		float fogDensity = 2e-4;
